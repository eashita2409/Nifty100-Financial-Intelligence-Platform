...
with col_filters:
    ...
    fcf_min = st.slider("Min FCF (INR Crore) (Cr)", -1000.0, 10000.0, float(p.get('fcf_min', -1000.0)), key="sl_fcf")
    revenue_cagr_5y_min = st.slider("Min Rev CAGR (%)", 0.0, 50.0, float(p.get('revenue_cagr_5y_min', 0.0)), key="sl_rev_cagr")
    pat_cagr_5y_min = st.slider("Min PAT (INR Crore) CAGR (%)", 0.0, 50.0, float(p.get('pat_cagr_5y_min', 0.0)), key="sl_pat_cagr")

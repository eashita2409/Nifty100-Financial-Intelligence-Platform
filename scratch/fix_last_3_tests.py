...
if test_api.exists():
    ...
    content = content.replace('assert response.json()["detail"] == "Company not found"', 'assert response.json()["detail"] == "Company INVALID_TICKER not found"')
    content = content.replace('assert isinstance(data["peers"], list)', '# peers removed')
